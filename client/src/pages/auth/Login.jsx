...
function Login() {
  ...
  const handleSendOtp = async (e) => {
    e.preventDefault();
    const loginField = loginData.email || loginData.phone;
    if (!loginField) {
  ...
  const handleVerifyOtp = async (e) => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        else if (response.data.user.role === "delivery") navigate("/delivery/dashboard");
        else navigate("/");
      }
    } catch (err) {
  ...
  const handleBack = () => { setStep(1); setOtp(""); setError(""); };
        ...
        {step === 1 && (
          <>
            ...
            <form onSubmit={handleSendOtp}>
              <div className="mb-4">
                ...
                <div className="relative">
                  ...
                  <input type="email" name="email" className="input-field pl-12" 
                    placeholder="Enter your email" value={loginData.email} onChange={handleChange} />
                </div>
              </div>
              ...
              <div className="mb-4">
                ...
                <div className="relative">
                  ...
                  <input type="tel" name="phone" className="input-field pl-12"
                    placeholder="Enter phone" value={loginData.phone} onChange={handleChange} />
                </div>
              </div>
            ...
            <p className="text-center mt-4 text-gray-600">
              No account? <Link to="/signup" className="text-primary font-medium">Contact Admin</Link>
            </p>
          </>
        )}
        ...
        {step === 2 && (
          <>
            <div className="text-center mb-4">
              ...
              <p className="auth-subtitle">Enter code sent to<br /><strong>{userEmail}</strong></p>
            </div>
            {error && <div className="alert alert-danger">{error}</div>}
            {success && <div className="alert alert-success">{success}</div>}
            <form onSubmit={handleVerifyOtp}>
              <div className="mb-4">
                <input type="text" className="input-field text-center text-2xl tracking-widest"
                  placeholder="000000" value={otp} onChange={handleOtpChange} maxLength={6} />
              </div>
            ...
            <div className="text-center mt-4">
              {resendTimer > 0 ? (
                <p className="text-gray-500">Resend in <span className="font-medium">{resendTimer}s</span></p>
              ) : (
